...
namespace ReadlnLibrary.Core.Collections
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        public bool Remove(TElement item)
        {
            ...
            // if grouping key value changed
            if (group == null || !group.Contains(item))
            {
                group = this.TryFindGroup(item);
            }
        ...
        private Grouping<TKey, TElement> TryFindGroup(TElement item)
        {
            var group = this.FirstOrDefault(g => g.Contains(item));
            return group;
        }
